...
EsIndexer_Copy_Configuration() {
    ...
    local source_dir="$HOME/mvx-api-deployer/config/$NETWORK/1-mx-chain-es-indexer-go"
    local repo_dir="$HOME/mx-chain-es-indexer-go"
...
EsIndexer_Create_Service() {
    ...
    sudo cp "$HOME/mvx-api-deployer/config/$NETWORK/1-mx-chain-es-indexer-go/mvx-elasticindexer.service" "$service_file" || {
